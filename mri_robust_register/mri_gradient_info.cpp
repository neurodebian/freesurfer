...
//static char vcid[] = "$Id: mri_gradient_info.cpp,v 1.3.2.1 2011/08/23 18:53:41 mreuter Exp $";
char *Progname = NULL;

double scanX(MRI* mri_grad, MRI* mri_mask)
{
 if(mri_mask) assert(mri_grad->width  == mri_mask->width);
 if(mri_mask) assert(mri_grad->height == mri_mask->height);
 if(mri_mask) assert(mri_grad->depth  == mri_mask->depth);
		  ...
		  if (lastg < 0)
			...
			{ 
			  ...
			  if (nextg > currg && currg < max) // going up after passing max
				{
				  if (right < 0) right = w-1; // else right was set when going sideways
				...
				// finally set new max
			  if (nextg > max) max = nextg; // store higher max
			}
	    ...
	    if (mri_mask!=NULL)
		  ...
		  if (lastg < 0)
			...
			{ 
			  ...
			  if (nextg > currg && currg < max) // going up after passing max
				{
				  if (right < 0) right = h-1; // else right was set when going sideways
	    ...
	    if (mri_mask!=NULL)
		  ...
		  if (lastg < 0)
			...
			{ 
			  ...
			  if (nextg > currg && currg < max) // going up after passing max
				{
				  if (right < 0) right = d-1; // else right was set when going sideways
...
double scanXold(MRI * mri_in, MRI* mri_grad, MRI* mri_mask)
{
 ...
 if(mri_mask) assert(mri_in->width == mri_mask->width);
 assert(mri_in->height == mri_grad->height);
 if(mri_mask) assert(mri_in->height == mri_mask->height);
 assert(mri_in->depth == mri_grad->depth);
 if(mri_mask) assert(mri_in->depth == mri_mask->depth);
...
//h=128;
	{
	  ...
	  for (w = 0; w < mri_in->width; w++)
	  {
        ...
        if ( (curri > lasti && curri > nexti) || (curri < lasti && curri < nexti))
				{
				   edgecount++;
			...
			{
			  if ( curri > lasti && curri > nexti) extremum = w-1;
			  if ( curri < lasti && curri < nexti) extremum = w-1;
			
			  if (currg > lastg && currg > nextg && currg > thres) onedge = true;
			}
...
//    cout << endl;
  ...
  MRI* mri_in = MRIread(mrif.c_str());
	...
	MRI* mri_mask = NULL;
	if (argc >= 3) mri_mask = MRIread(argv[2]);
