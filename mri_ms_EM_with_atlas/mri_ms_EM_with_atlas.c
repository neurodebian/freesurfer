...
// mri_ms_EM_for_MEF_with_atlas.c
...
main(int argc, char *argv[]) {
  ...
  nargs = handle_version_option (argc, argv, "$Id: mri_ms_EM_with_atlas.c,v 1.5 2011/03/02 00:04:23 nicks Exp $", "$Name: stable5 $");
  ...
  if (nvolumes <= 2) {
    ...
    for (z=0; z < depth; z++)
      for (y=0; y< height; y++)
        for (x=0; x < width; x++) {
          if (MRIvox(mri_mask,x,y,z) == 0) continue;
          if (MRIFvox(mri_prior[1], x, y, z) > 0.1) continue;
...
# else
    for (z=0; z < depth; z++)
      for (y=0; y< height; y++)
        for (x=0; x < width; x++) {
          if (MRIvox(mri_mask,x,y,z) == 0) continue;
          if (MRIFvox(mri_prior[2], x, y, z) < 0.5) continue;
          val1 = MRIFvox(mri_flash[0], x, y, z);
  ...
  for (z=0; z < depth; z++)
    for (y=0; y< height; y++)
      for (x=0; x < width; x++) {
        if (MRIvox(mri_mask, x, y, z) == 0) continue;
        if (MRIFvox(mri_mem[0], x, y, z) > MRIFvox(mri_mem[2], x, y, z)) continue; //already CSF

        value = MRIFvox(mri_flash[0], x, y, z);
        if (value < mean1 || value > mean2) continue;
    ...
    for (z=0; z < depth; z++)
      for (y=0; y< height; y++)
        for (x=0; x < width; x++) {
          ...
          if (value < -0.4 ) value = -0.4; //this is wrong! Many places are negative
          if (value > 254.4) value = 254.4;
